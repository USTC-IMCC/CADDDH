...
def compute(name):
    ...
    with  open('./Dataset/txt/'+str(name)+'.txt','r') as fileReader:
    ...
    image = cv2.imread('./Dataset/pic/'+str(name)+'.jpg')
    sp = image.shape
    im = Image.open('./Dataset/pic/' + str(name) + '.jpg')    
    #1 point     
    x1=float(line[0])
